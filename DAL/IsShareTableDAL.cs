...
namespace DAL
{
    public class IsShareTableDAL
    {
        public IsShareTable GetShareTable(string openId, string actName)
        {
            ...
            return DapperHelper<IsShareTable>.Query(sql, new { openId, actName }).FirstOrDefault();
        }

        public int AddShareLog(string openId,string actName)
        {
            string sql = "insert into IsShareTable(OpenId,IsShare,ActivityName,CreateTime) values(@openId,1,@actName,getdate())";
